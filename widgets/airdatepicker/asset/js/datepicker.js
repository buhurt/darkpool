!function (t, e, i) {
    !function () {
        /* Функция что бы в вызове компонента можно было указывать 2 аттрибуда вроде date_from, date_to
         Во всех сёрч моделях в запросах использовались два аттрибута, типа дата_от и дата_до
         когда у нас в компоненте стоит 'range' => true, он будет отравлять date_range - две даты в одной строковой
         переменной с разделителем (пример 'multipleDatesSeparator' => ' - ') (в clientOptions),
            Что бы можно было использовать компонент как раньше(с двумя аттрибутами), в опциях компонента ставим
         attribute1 => 'какая_то_там_дата_from' и attribute2 => 'какая_то_там_дата_to'.
          Добавляется два невидимых поля, и при выборе диапазона дат, прям на фронтенде разбивается дата по
          разделителю, что бы бекенде было два аттрибута */
        var onSelectFunction = function (formattedData, date, inst, range = this.range, separator = this.multipleDatesSeparator) {

            if (range) {
                if ($('#' + inst.el.id).val().match(/01.1970/g)) {
                    $('#' + inst.el.id).val($('#' + inst.el.id).attr('value'));
                }
                dates = formattedData.split(separator);
                attribute1id = $('#' + inst.el.id).attr('attribute1id');
                attribute2id = $('#' + inst.el.id).attr('attribute2id');

                /* Из диапазона подставили дату_от */
                $('#' + attribute1id).val(dates[0]);

                if (dates[1] === undefined) {
                    /* Если у DatePickerWidget стоит 'range' => true, то вместо 2х дат - даты_от и даты_до
                      пользователь может выбрать только одну дату(дату_от). Такой выбор воспринимается пользователем
                      ни как диапазон, а как поиск по одной конкретной выбранной дате. В таких случаях в дату_до,
                      мы подставим дату_от. У нас будет диапазон от одной даты до неё же, что тождественно и диапазону и
                      выбору конкретной даты.
                       Вот например в сёрчМОдели фильтруем по диапазону.
                       if(!empty($this->date_backup_from)) $query->andFilterWhere(['>=', 't.date_backup', Date('Y-m-d 00:00:01', strtotime($this->date_backup_from))]);
                       if(!empty($this->date_backup_to)) $query->andFilterWhere(['<=', 't.date_backup', Date('Y-m-d 23:59:59', strtotime($this->date_backup_to))]);
                       А потом пользователь выбирает только одну дату_от и ждёт что поиск будет происходить, по конкретной дате, в итоге по одной дате и будет искать
                       При этом в запрос добавлять третий вариант фильтрации не нужно.
                      */
                    $('#' + attribute2id).val(dates[0]);
                } else {
                    /* Из диапазона подставили дату_до */
                    $('#' + attribute2id).val(dates[1]);
                }
            }
        }
